...
//! nothing in the protection plane. Breaker failure arms on demand.
...
fn two_networks_two_blast_radii() {
    ...
    // Lose PTP lock: differential bound-stops (cross-bay comparison needs
    // matching sync) while overcurrent keeps protecting on magnitude alone.
    by_name("PTP_SLAVE")
        .input
    ...
    let trip = writes_of("signals::TRIP");
    advance_ms(1000);
    assert!(
        writes_of("signals::TRIP") > trip + 2,
